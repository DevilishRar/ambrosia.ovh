...
const APPLICATION_ID = process.env.DISCORD_APPLICATION_ID || '1539701435237662830';
    ...
    if (customId.startsWith('verify_tx_modal_')) {
      ...
      if (isTestTx) {
        ...
        if (channelId2) {
          ...
          await sendMessage(BOT_TOKEN, channelId2, testMsg);
        }
      ...
      try {
        ...
        for (var n = 0; n < Math.min(MONERO_NODES.length, 1) && !txResult; n++) {
          try {
        ...
        if (!txResult) {
          verifyResult = '\u274C Transaction `' + txHash.substring(0, 16) + '...` not found. It may still be propagating. Try again in a few minutes.';
        } else if (!verified) {
          verifyResult = '\u23F3 Transaction `' + txHash.substring(0, 16) + '...` not yet confirmed. Confirmations: ' + (txResult.confirmations || 0) + '. Please wait.';
        } else {
          var totalPiconero = 0;
          ...
          verifyResult = '\u2705 TX verified! `' + totalXmr.toFixed(6) + ' XMR` (~$' + totalUsd.toFixed(2) + ' USD).';
          if (CUSTOMER_ROLE_ID && targetUserId && targetUserId.length >= 17) {
            verifyResult += ' **Verified Customer** role assigned to <@' + targetUserId + '>.';
          }

          if (channelId2) await sendMessage(BOT_TOKEN, channelId2, verifyResult);
        }
      } catch (e) {
        console.error('[Ambrosia] TX verification error:', e.message);
        verifyResult = '\u274C Error verifying transaction. Please try again or contact staff.';
      }

      return res.json({ type: 4, data: { content: verifyResult || '\u274C Verification failed.', flags: 64 } });
    }
